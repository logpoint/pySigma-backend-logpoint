...
from sigma.pipelines.common import logsource_windows_process_creation, logsource_windows_registry_add, \
    logsource_windows_file_rename, logsource_windows_file_access, logsource_windows_file_delete, \
    logsource_windows_file_event, logsource_windows_file_change, logsource_windows_registry_event, \
...
from sigma.pipelines.logpoint.logpoint_mapping import logpoint_windows_sysmon_variable_mappings, \
    logpoint_windows_security_audit_mapping, logpoint_windows_service_control_manager_mapping, \
    logpoint_windows_common_taxonomy, logpoint_windows_sysmon_mapping, windows_sysmon_label_mapping
...
class SnakeCaseMappingTransformation(FieldMappingTransformationBase):
    ...
    _re_to_snake_case: ClassVar[Pattern] = re.compile('([a-z0-9]+|[A-Z][a-z0-9]+|[A-Z0-9]+)')
            ...
            if isinstance(val, tuple):
                for item in val:
                    processing_item = ProcessingItem(
                        ...
                        rule_conditions=[LogsourceCondition(product="windows", category=category)],
                    )
        ...
        + [
              ...
              + [
                ProcessingItem(
                    ...
                    transformation=FieldMappingTransformation(logpoint_windows_sysmon_mapping),
                    ...
                    rule_conditions=[
                        ...
                        LogsourceCondition(product="windows", category="process_termination"),
                   ...
                   ProcessingItem(
                       ...
                       rule_conditions=[LogsourceCondition(product="windows", service="security")],
                   )
              ]
              + [
              ...
              + [
                  ProcessingItem(  # Logpoint's Security Audit Field mappings
                      ...
                      rule_conditions=[LogsourceCondition(product="windows", service="system"),
                                       RuleContainsDetectionItemCondition(field="Provider_Name",
              ...
              + [
                  ProcessingItem(  # Logpoint's Security Audit Field mappings
                      ...
                      rule_conditions=[LogsourceCondition(product="windows", service="powershell-classic")]
                  ),
